...
public class Proficiency : MonoBehaviour {
	...
	public void UpdateProficiency() {
		...
		DbSetup.UpdateTableField("Proficiencies",
								 "ProficiencyNames",
								 newName,
								"ProficiencyNames = " + DbSetup.GetParameterNameFromValue(CurrentProficiencyName),
								CurrentProficiencyName);
		DbSetup.UpdateTableField("Proficiencies",
								 "Thresholds",
								 newThreshold,
								 "ProficiencyNames = " + DbSetup.GetParameterNameFromValue(newName),
								newName);
		ActivateSelectorBtn();
	...
	public void DeleteProficiency() {
		...
		DbSetup.DeleteTupleInTable(	"Proficiencies",
								 	fields);
		Destroy(gameObject);
	}
